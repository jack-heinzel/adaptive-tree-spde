...
via an Adaptive Tree Hat Basis for the Mat\'ern SPDE}
\author{Jack Heinzel}
\date{Draft --- \today}
...
The central object of this note is an \emph{adaptive tree hat basis} on
$[0,1]^D$: a hierarchical, multi-resolution basis built from tensor products of
one-dimensional constant, linear, and dyadic hat functions, refined where the
observed point pattern is dense.  This basis yields finite element (FEM) mass
and stiffness matrices in closed form --- the stiffness matrix is diagonal and
the mass matrix has a recursive block structure --- and, crucially, refines
\emph{locally} ($O(D)$ work per node) without rebuilding any global mesh.  The
field is assembled with FEM and sampled jointly with the hyperparameters
$(\kappa, \sigma, \mu)$ using gradient-based MCMC (NUTS in BlackJAX, plus
dimension-robust pCN and Hilbert-space HMC), with reversible-jump moves over the
refinement tree.
A Delaunay triangulation pipeline is retained as the Level-1 baseline
(Section~\ref{sec:baseline}); the lessons appendix records why the tree basis
superseded it.
We also discuss several implementation subtleties: the domain-area correction
for buffer nodes, an $O(n)$ eigenvalue trick for the log-determinant, and a
centred parameterisation that eliminates Neal's funnel.
\end{abstract}
...
$\{\phi_j\}$ is constructed.  The concrete closed forms for $C$ and $G$ used
throughout this note come from the adaptive tree hat basis of
Section~\ref{sec:hat-tree}; the Delaunay-triangulation baseline of
Section~\ref{sec:baseline} supplies an alternative set of the same matrices.
...
for the adaptive tree basis are given in
Sections~\ref{sec:hat-mass}--\ref{sec:hat-stiffness}
(and for the triangulation baseline in Section~\ref{sec:baseline}).
...
The RJMCMC sampler of Section~\ref{sec:rjmcmc} does not build $\mathcal{A}$
directly; it grows a \emph{birth tree} $\mathcal{T}$ whose nodes are sequences
...
\end{equation}
With \eqref{eq:canon-birth-rule} the RJMCMC birth/death moves of
Section~\ref{sec:rjmcmc} traverse each $\phi_s$ through exactly one path, so the
move counts $N_m,M_m$ and the acceptance ratio no longer double-count
...
function at $s_i$.  For the adaptive tree basis this is the tensor-product form
\eqref{eq:hat-loglik-new}; for the triangulation baseline the row reduces to the
barycentric coordinates of the containing simplex, as follows.
...
reversible-jump MCMC \citep{green1995}.  For the adaptive tree basis of
Section~\ref{sec:hat-tree} a \emph{birth move} advances one node one step in its
local hierarchy (const $\to$ linear $\to$ unit hat $\to$ sub-hat), adding one
coefficient; a \emph{death move} reverses it.  Because sibling hats have zero
mass and stiffness overlap (Eq.~\eqref{eq:C-block-zero}), each move updates the
FEM matrices in $O(D)$ work with no global rebuild.  We first present the scheme
in its original form on the triangulation baseline, where a node corresponds to
a triangle and a birth performs a ternary simplex split; the acceptance ratio
below is identical for the tree basis, with the local FEM updates replaced by
the closed forms of Section~\ref{sec:hat-tree}.
...
(Section~\ref{sec:spde-op}), the same argument holds with $\kappa^2$
...
behind the general FEM framework of Section~\ref{sec:galerkin}, and because the
likelihood, hyperparameter, anisotropic, and RJMCMC machinery were all first
developed on it.
