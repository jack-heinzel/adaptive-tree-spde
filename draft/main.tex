...
The Level-1 method (Section~\ref{sec:triangulation}) builds a Delaunay
...
The number of Delaunay simplices needed to triangulate $[0,1]^D$ grows at
least as fast as $D!$ in the number of vertices (and this lower bound is
believed to be tight, though the precise asymptotics remain an open problem in
combinatorics~\citep{devadoss2011discrete}).  Concretely, a regular grid of
$n^D$ points in $D$ dimensions produces $O(n^D \cdot D!)$ simplices: already
for $D=4$ and a modest $n=10$ this yields millions of simplices.  FEM matrix
assembly over all of them is prohibitive.
...
Section~\ref{sec:adaptive-tri}.

\subsection{Hat-Function Tree Basis as the Natural Replacement}
\label{sec:lesson-tree-basis}

The adaptive hierarchical basis (Section~\ref{sec:adaptive-tri}) resolves both
obstacles above.
