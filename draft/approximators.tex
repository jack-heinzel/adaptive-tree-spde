...
The fix is identical to Section~\ref{sec:uniqueness}: impose the total order
$1<2<\cdots<D$ and keep the canonical (sorted) split history, equivalently demote
the highest-indexed cut via the canonical parent \eqref{eq:canon-parent}.  The
state-local admissibility rule \eqref{eq:canon-birth-rule} carries over verbatim
--- within a cell's split lineage, a cut in dimension $d$ is allowed only if no
higher-indexed dimension has been cut --- making the tree-to-partition map a
bijection and removing the double-counting from the RJMCMC move counts.
